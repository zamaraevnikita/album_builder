            ...
            <ul className="space-y-1" ref={layersListRef}>
              {currentElements.slice().reverse().map(layer => (
                ...
                >
                  ...
                  {layer.groupId && (
                    <span className="ml-2 text-[9px] bg-gray-200 px-1 rounded text-gray-500">GRP</span>
                  )}
                  <button
                    onClick={e => {
                      e.stopPropagation();
              ...
              {currentElements.length === 0 && (
                <div className="text-xs text-gray-300 text-center py-8">No layers on this page</div>
              )}
            </ul>
          </div>
        )}
